    ...
    if st.button("🚀 AI解析開始 (Gemini 2.5)"):
        with st.spinner("AIが画像を解析し、食レポを執筆中...（これには数秒かかります）"):
            try:
                ...
                # --- Plan Logic (Feature Guard) ---
                # デモ用にサイドバーでプランを強制上書きできるようにする（本来はDBから取得）
                # Default is 'standard' (69,800) to show the value first (Puppy Dog)
                
                # 実際のストアプラン取得
                db_plan = "standard"
                if store_name_input:
                    # 簡易取得 (store_idが確定していないフローもあるため、再検索コスト許容)
                    try:
                        res = supabase.table("stores").select("plan_code").eq("store_name", store_name_input).execute()
                        if res.data:
                            db_plan = res.data[0].get("plan_code", "standard")
                    except:
                        pass
                
                # デバッグ用トグル (要件: "39に比べて69がどれだけ便利か？の差別化が必要")
                # ユーザーが体験できるように、開発中はサイドバーで切り替え可能にする
                st.sidebar.markdown("---")
                st.sidebar.subheader("💎 Plan Simulation")
                simulated_plan = st.sidebar.select_slider(
                    "プランによる機能差分 (Simulation)",
                    options=["economy", "standard"],
                    value=db_plan if db_plan in ["economy", "standard"] else "standard",
                    format_func=lambda x: "39,800円 (OCRのみ)" if x == "economy" else "69,800円 (AI創作)"
                )
                
                is_economy = (simulated_plan == "economy")

                # プロンプトの構築
                if is_economy:
                    # Plan 39: OCR Only Mode
                    # 食レポなどの創作を禁止し、事実のみ抽出する
                    prompt_text = f"""
                    あなたはデータ入力アシスタントです。
                    このメニュー画像を分析し、記載されている料理名、価格、カテゴリ、アレルゲンを正確に抽出してください。
                    
                    # 制約
                    - **「食レポ」や「説明文」の創作は一切行わないでください。** (空欄にする)
                    - 記載されている文字情報をそのままデジタル化することを目的とします。
                    ...
                    1. メニュー名 (画像から読み取ったまま)
                    2. 価格 (数字)
                    3. カテゴリ (ドリンク/フード/ランチ/コース/デザート)
                    4. アレルゲン推測 (推測可能ならTrue)
                    5. **説明文 (menu_content/description_rich) は必ず空文字 "" にしてください。**

                    出力は指定されたJSON形式で行ってください。
                    """
                else:
                    # Plan 69: Full AI Creative Mode
                    prompt_text = f"""
                    ...
                    4. 【重要】日本語の説明文（menu_content）:
                       - **ここがあなたの腕の見せ所です。** 読んだ人が「食べたい！」と思うような「食レポ」調で書いてください。
                ...
                if store_name_input:
                    try:
                        with st.spinner("💾 データベースに保存中..."):
                            ...
                            # 2. メニュー保存 (Payment Guard Check)
                            if store_id:
                                # Payment Guard Initation
                                ...
                                # Limit Check (Partial Save)
                                # リスト丸ごと保存ではなく、残枠分だけ保存する "Puppy Dog Close"
                                remaining = status.get("remaining", 9999)
                                ...
                                # 保存対象の絞り込み
                                if len(clean_items) > remaining:
                                    items_to_save = clean_items[:remaining]
                                    items_discarded = len(clean_items) - remaining
                                    st.toast(f"⚠️ 無料枠はあと {remaining} 品です。{items_discarded} 品は保存されませんでした。")
                                    # 保存処理
                                    save_menu_to_db(store_id, items_to_save, selected_persona)
                                    # Upsell Message
                                    guard.render_upsell_message()
                                else:
